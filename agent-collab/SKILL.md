...
**Cap the rounds — converge or escalate.** The rounds are a *forward progression*, not a loop: A → B → A → B → A, roughly five turns to a decision. If the same point keeps getting re-litigated past ~3 round-trips without converging, **stop** — don't keep waking each other. Record the best decision you have, cut a follow-up task, or escalate to the human with the *one* blocking decision. An unbounded collab is the expensive failure mode: two agents re-waking each other on a growing thread, never concluding, quietly burning a fortune. If your setup wakes agents automatically, enforce this ceiling *mechanically* — cap how many times a single thread can reopen, then force-conclude and escalate — because "the agents will wrap up on their own" is a guideline, and a guideline is not a limit.

**Research is free, within a turn.** Round 3 is the designated research round, but an agent may spin up sub-agents or do real research (read a file, run a query, check live state) at *any* step when it makes the response materially better. That work happens inside the turn; it doesn't add rounds.

**Get the spec approved before delegating.** When convergence produces a spec or plan with work to hand off, the default is: post it to the human and get their approval *before* cutting execution tasks. Delegate immediately only if the human already approved or told you to proceed; if you're unsure whether to wait, ask. Spawning execution work off an unapproved spec is how a confidently-wrong plan becomes shipped mistakes.
...
| Let a collab run unbounded — keep waking each other until it converges "eventually" | Two agents re-waking on a growing thread rarely converge on their own; the thread balloons and cost with it. This is the single most expensive failure mode. | Cap it at ~5 turns / ~3 round-trips. Converge, or escalate to the human with the one blocking decision — and enforce the cap mechanically if wakes are automatic. |
| Cut execution tasks straight off an unapproved spec | A confidently-wrong plan becomes shipped work before anyone with authority saw it. | Default to getting the human's approval of the spec before delegating; proceed unprompted only if they already approved. |
